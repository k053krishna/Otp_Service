...
@Service
public class HmacOneTimePasswordGenerator implements IHmacOneTimePasswordGenerator {
	 ...
	 *                       algorithms
	 *
	 * @throws UncheckedNoSuchAlgorithmException if the given algorithm is not
	 *                                           supported by the underlying JRE
	 */
	HmacOneTimePasswordGenerator(final int passwordLength, final String algorithm)
	...
	private Mac getMac() {
		try {
			// Cloning is generally cheaper than `Mac.getInstance`, but isn't GUARANTEED to
			// be supported.
			...
			try {
				...
				// This should be impossible; we're getting the algorithm from a Mac that
				// already exists, and so the
	...
	/**
	 * Generates a one-time password using the given key and counter value and
	 * formats it as a string using the system default locale.
	 *
	 * @param key     the key to be used to generate the password
	 * @param counter the counter value for which to generate the password
	 *
	...
	/**
	 * Generates a one-time password using the given key and counter value and
	 * formats it as a string using the given locale.
	 *
	 * @param key     the key to be used to generate the password
	 * @param counter the counter value for which to generate the password
